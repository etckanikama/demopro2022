    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                #     print("x1,y1, x2,y2 ",detx1, dety1, detx2,dety2)
                        # # boundinx box内にアクセス
                        # for x in range(detx1, detx2):
                        #     for y in range(dety1, dety2):
                        #         pass

                    # if det[:, -1] == 0.0: #ラベルが人だったら
                    #     detx1 = int(det[0][0].numpy())
                    #     dety1 = int(det[0][1].numpy())
                    #     detx2 = int(det[0][2].numpy())
                    #     dety2 = int(det[0][3].numpy())
            ...
            if view_img:
                ...
                if 0 in det[:,-1]:
                    ...
                    # personのboxの画素数を算出
                    box_width = detx2 - detx1
                    box_heigh = dety2 - dety1
                    box_pixel_size = box_heigh * box_width

                    # person内を探索：白(255)をカウント
                    white_count = 0 
                    for x in range(detx1,detx2):
                        for y in range(dety1,dety2):
                            # print("aaaaa",im0[y][x])
                            if im0[y][x] == 255: #opencvの画像のアクセスはy,x
                                white_count += 1
                                # print(x,y)
